    ...
    if request.method == "POST":
        if "signup-form" in request.POST:
            ...
            if new_login is None:
                ...
                global_code = code
                send_mail(
                    "Password Manager: confirm email",
...
class AutoFillAll(ListAPIView):
    queryset = Password.objects.all()
    serializer_class = PasswordSerializer
    
    
class AutoFill(ListAPIView):
    serializer_class = PasswordSerializer

    def get_queryset(self):
        queryset = Password.objects.all()
        name = self.request.query_params.get('name')  # Retrieve the 'email' query parameter
